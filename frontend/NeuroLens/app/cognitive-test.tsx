...
import { 
  View, Text, TouchableOpacity, StyleSheet, SafeAreaView, Dimensions, ScrollView 
} from 'react-native';
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {gameState === "intro" && (
          <View style={styles.centerContainer}>
            ...
            <Text style={styles.subtitle}>Memorize the highlighted numbers.</Text>
        ...
        {(gameState === "memorize" || gameState === "recall") && (
          <View style={styles.gameContainer}>
            <Text style={styles.statusText}>
              {gameState === "memorize" ? `Memorize! (${timeLeft}s)` : "Your Turn"}
            </Text>
            ...
            <View style={styles.grid}>
              {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => {
                ...
                return (
                  ...
                  >
                    <Text style={[styles.cellText, (isHigh || isSel) && {color: '#fff'}]}>
                      {num}
                    </Text>
                  </TouchableOpacity>
                );
        ...
        {gameState === "result" && (
          <View style={styles.centerContainer}>
            {userInput.length === sequence.length ? (
              ...
              <>
                <View style={[styles.circleIcon, { backgroundColor: '#F44336' }]}>
                  <Ionicons name="close" size={50} color="#fff" />
                </View>
                <Text style={styles.title}>Game Over</Text>
                <Text style={styles.subtitle}>Sequence: {sequence.join(" - ")}</Text>
...
const styles = StyleSheet.create({
  ...
  statBox: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#1E1E1E', padding: 10, borderRadius: 12 },
  ...
  circleIcon: { width: 100, height: 100, borderRadius: 50, alignItems: 'center', justifyContent: 'center', marginBottom: 20 },
  title: { fontSize: 28, fontWeight: 'bold', color: '#fff', marginBottom: 10 },
  subtitle: { fontSize: 16, color: '#aaa', marginBottom: 30 },
  primaryButton: { backgroundColor: '#4CAF50', paddingVertical: 15, paddingHorizontal: 40, borderRadius: 30 },
  secondaryButton: { borderWidth: 1, borderColor: '#666', paddingVertical: 15, paddingHorizontal: 30, borderRadius: 30 },
  btnText: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
  secBtnText: { color: '#ccc', fontSize: 18, fontWeight: 'bold' },
  instructions: { marginTop: 40, backgroundColor: '#1E1E1E', padding: 20, borderRadius: 16, width: '100%' },
  instTitle: { color: '#fff', fontWeight: 'bold', marginBottom: 10 },
  instText: { color: '#aaa', marginBottom: 5 },
  gameContainer: { alignItems: 'center' },
  statusText: { color: '#4CAF50', fontSize: 20, fontWeight: 'bold', marginBottom: 20 },
  grid: { flexDirection: 'row', flexWrap: 'wrap', width: width - 40, justifyContent: 'center', gap: 10 },
