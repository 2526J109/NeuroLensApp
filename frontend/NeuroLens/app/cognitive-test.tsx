...
export default function CognitiveAssessment() {
  ...
  const handleSdmt = useCallback((digit:number)=>{
    ...
    const trial:SdmtTrial={
      trial:currentIdx, symbolIndex:si,
      response:digit, correct:ok,
      rt, timestamp:now,
    };
  ...
  const submit = useCallback(async ()=>{
    ...
    try {
      const response = await api.post('/api/cognitive-analysis/predict', {
        ...
        SEX:          mappedSex,
        fampd:        0,  // hardcoded until signup questions added
        rem:          0,  // hardcoded until signup questions added
      });
...
const styles = StyleSheet.create({
  ...
  infoScreen: {
    ...
    paddingBottom:28, justifyContent:"center", gap:14,
  },
  taskScreen: {
    flex:1, paddingHorizontal:16, paddingTop:12, paddingBottom:14,
  },
  sdmtScreen: {
    flex:1, paddingHorizontal:PAD, paddingTop:12,
    paddingBottom:16, flexDirection:"column",
  },
  canvas: {
  ...
  iconCircle: {
    width:68, height:68, borderRadius:34, backgroundColor:C.greenLt,
  ...
  listBox: {
    backgroundColor:C.white, borderRadius:18, padding:16, gap:0,
  ...
  exampleRow:{ flexDirection:"row", alignItems:"center", backgroundColor:C.greenLt, borderRadius:14, padding:14, gap:12 },
  exDot: {
    width:46, height:46, borderRadius:23, backgroundColor:C.greenLt,
  ...
  keyOuter: {
    backgroundColor:C.subtle, borderRadius:14, paddingVertical:10, paddingHorizontal:4,
  },
  keyLabel: {
