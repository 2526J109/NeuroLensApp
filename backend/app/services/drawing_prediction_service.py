...
from app.utils.normquadstream_features import extract_normquadstream_features, features_to_dict
from app.models.drawing_normquadstream_predictor import predict_normquadstream_risk
...
def analyze_with_normquadstream(
    user_id: str,
    drawing_data: Dict[str, Any],
    session_id: str = None,
) -> Dict[str, Any]:
    """
    Extract 12 NB22 scale-invariant features from mobile drawing data,
    run NormQuadStream inference with MC Dropout, and save to Firestore.

    Input format (same as analyze-local):
        spiral_data.points: [{x, y, timestamp(ms)}, ...]
        wave_data.points:   [{x, y, timestamp(ms)}, ...]
    """
    spiral_points = (drawing_data.get("spiral_data") or {}).get("points") or []
    wave_points   = (drawing_data.get("wave_data")   or {}).get("points") or []

    s1, s2, s3, s4 = extract_normquadstream_features(spiral_points, wave_points)

    prediction = predict_normquadstream_risk(s1, s2, s3, s4)
    prediction["nqs_features"] = features_to_dict(s1, s2, s3, s4)

    prediction_to_save = {
        k: v for k, v in prediction.items()
        if k not in ("nqs_features", "source")
    }
    save_result = save_prediction_for_user(user_id, prediction_to_save, session_id)
